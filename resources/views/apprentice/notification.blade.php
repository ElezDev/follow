  ...
  <header class="bg-white text-[#009e00] px-5 py-2.5 flex flex-col items-center border-t-[5px] border-t-white border-b border-b-[#e0e0e0]">
        ...
        <div class="flex justify-between w-full">
            ...
            <div class="relative flex items-center ml-auto">
                ...
                <div class="relative">
                    <!-- Imagen de usuario -->
                    <p class="text-sm font-bold mr-[16px] bg-[#f5f4f4] p-1 rounded-lg  z-20"></p>
                     ...
                     <div class="p-4">
                         <div class="flex items-center mb-4">
                             <div>
                                 ...
                                 <p class="mt-2 text-sm">Aprendiz</p>
                             </div>


                         </div>
                         <ul>
                             <li class="mt-2"><a href="{{ route('apprentice.profile') }}" class="block py-1 font-bold text-center text-green-600 bg-white border border-green-600 rounded-lg hover:text-white hover:bg-green-600">Ver perfil</a></li>

                             <li class="mt-2"><a href="{{ route('apprentice.settings') }}" class="block p-2 text-black rounded-lg hover:bg-white">Configuración</a></li>
                         </ul>
    ...
    <nav class="bg-[#009e00] px-2.5 h-14 py-1.5 flex justify-start items-center relative z-10">
        ...
        <div class="flex justify-center w-full">
            <ul class="flex justify-center space-x-4 horizontal-list" >
                <li>
                    <a href="{{ route('apprentice.home') }}" class="block px-4 py-2 text-center text-white transition bg-transparent rounded-lg hover:bg-green-700">
                        Inicio
                    </a>
                </li>
                <li>
                    <a href="{{ route('apprentice.calendar') }}" class="block px-4 py-2 text-center text-white transition bg-transparent rounded-lg hover:bg-green-700">
                        Calendario
                    </a>
                </li>
            </ul>
...
<div class="container p-4 mx-auto">
    <div class="bg-white divide-y divide-gray-200 rounded-lg shadow">
        @forelse($notificaciones as $notificacion)
            <div class="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50" onclick="mostrarModal({{ json_encode($notificacion) }})">
                <div>
                    <h3 class="text-lg font-semibold">{{ $notificacion['message'] ?? 'Mensaje no disponible' }}</h3>
                    <p class="text-sm text-gray-500">{{ $notificacion['content'] ?? 'Contenido no disponible' }}</p>
                    <p class="text-xs text-gray-400">Fecha: {{ $notificacion['shipping_date'] ?? 'Fecha no disponible' }}</p>
                </div>
...
<script>
    function mostrarModal(notificacion) {
        ...
        document.getElementById('modal-sender-name').textContent = `${sender.name ?? 'Nombre no disponible'} ${sender.last_name ?? ''}`;
